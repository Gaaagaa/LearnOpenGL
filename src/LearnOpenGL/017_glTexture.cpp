...
int main(int argc, char * argv[])
{
	...
	GLFWwindow * window = glfwCreateWindow(WIDTH, HEIGHT, "LearnOpenGL - 017 - glTexture", NULL, NULL);
	if (NULL == window)
	{
	...
	glfwSetWindowUserPointer(window, &alphaValue);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUseProgram(xshader.program_id());
		{
			...
			glUniform1f(glGetUniformLocation(xshader.program_id(), "alphaValue"), alphaValue);
...
void key_callback(GLFWwindow * window, int key, int scancode, int action, int mode)
{
	...
	else if ((GLFW_KEY_UP == key) && (GLFW_REPEAT == action))
	{
		GLfloat * alphaValue = (GLfloat *)glfwGetWindowUserPointer(window);
		if (NULL != alphaValue)
		{
			*alphaValue += 0.01F;
			if (*alphaValue > 1.0F)
	...
	else if ((GLFW_KEY_DOWN == key) && (GLFW_REPEAT == action))
	{
		GLfloat * alphaValue = (GLfloat *)glfwGetWindowUserPointer(window);
		if (NULL != alphaValue)
		{
