...
int main(int argc, char * argv[])
{
	...
	GLFWwindow * window = glfwCreateWindow(WIDTH, HEIGHT, "LearnOpenGL - 011 - glShader", NULL, NULL);
	if (NULL == window)
	{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		glUseProgram(xshader.program_id());
		{
			...
			GLfloat xoffset = sin(timeValue) / 2.0F;
			GLint xoffset_location = glGetUniformLocation(xshader.program_id(), "xoffset");
			glUniform1f(xoffset_location, xoffset);
